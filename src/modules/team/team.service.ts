...
import { player } from 'src/decorator/roles.decorator';
...
public async  uploadFile(file: Express.Multer.File, containerName: string) { 
  ...
  async saveUrl(createTeam: CreateTeamDto,file_url: string) { 
    const league = new League()
  ...
  async deleteTeam(id: number, containerName: string) {
    let getFile ='';
  ...
  //   return this.fileService.uploadFile(file, containerName)
  // }
